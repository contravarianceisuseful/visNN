...
public class NNBUilder : MonoBehaviour
{
    ...
    public static GameObject AxonPrefabRef { get; set; }
...
class BuilderEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Build"))
        {
            ...
            NNBUilder.AxonPrefabRef = builder.AxonPrefab;
            builder.Network.Build(builder.Inputs, builder.Outputs,
                builder.HiddenLayers, builder.HiddenLayerWidth, builder.HorSpacing, builder.VertSpacing,
                new System.Random());
            //FindObjectOfType<CameraResizer>().ResizeToFitAll();
        }

        if (GUILayout.Button("Propagate"))
        {
            var builder = target as NNBUilder;
            builder.Network.Propagate();
        }
    }
}
